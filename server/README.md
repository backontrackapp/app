...
| `BACKONTRACK_OPENAI_DAILY_TOKEN_LIMIT` | Default per-user assistant-token allowance for each local calendar day; used when no user override is set | `30000` |
...
`POST /assistant/respond` sends an authenticated, bounded conversation to the configured OpenAI Responses API and returns newline-delimited JSON. Displayable text arrives in `text_delta` records, followed by one authoritative `response` record containing the completed conversation items; failures that occur after streaming starts use an `error` record. The server fixes a flashcard-only instruction set: the assistant may work with the user's cards, Review sets, and review performance, and may generate supporting content only for those tasks. It refuses unrelated requests without invoking a tool. The server exposes only eight strict function schemas: list owned Review sets and settings, list and search owned cards, read cards and reviewer statistics from an owned set, propose a Review set creation, propose adding cards to an owned set, propose changing an owned set's name or review settings, propose card text updates, and present two to five answer choices. The client executes read calls only after filtering its synchronized data to the current owner. Choice calls render buttons and return the selected value to the model. Write calls pause for explicit user confirmation.

The OpenAI request uses `store: false`, disables parallel tool calls, includes an HMAC-derived safety identifier instead of an account ID, and requests a streaming response. The PHP proxy counts request input tokens, reserves the remaining per-user local-day token allowance before sending each provider request, and reconciles the reservation with the provider's completed-response `total_tokens` value. Each `users.assistant_daily_token_limit` is `NULL` by default and falls back to `BACKONTRACK_OPENAI_DAILY_TOKEN_LIMIT`; a non-`NULL` value is that user's allowance. The effective allowance is included in the assistant's instructions and the response output cap. Usage day and token total remain on the user row. The proxy forwards text deltas immediately with buffering and compression disabled, then derives tool calls and reasoning context only from the completed provider response. The API key never enters the web or native bundle. Assistant responses are rate limited to 30 requests per five minutes per account.

Confirmed create and add changes use `POST /assistant/flashcards/apply` before local bootstrap or the `flashcards.assistant_apply` sync command afterward. Both paths validate card limits and ownership again and create all cards plus the Review set relationship in one SQLite transaction. Confirmed name and settings updates use the existing optimistic Review set update path, which validates ownership and queues the same offline-first record patch as the editor. Assistant tools do not support archives, deletes, sharing changes, card-membership removal, or writes outside flashcards.
...
`slug` uses lowercase letters, numbers, and hyphens. `file` is a relative CSV path below the curated directory. `keywords` is pipe-separated. The optional `thumbnail` accepts an HTTPS URL or a relative JPEG, PNG, or WebP path below the curated directory. When present, the storefront shows that image instead of the card-image slideshow. Other optional catalog columns configure the cloned Review set: `mode`, `card_sides`, `indefinite`, `time_limit_seconds`, `max_cards`, `eject_behavior`, `eject_exclude_after`, `front_seconds`, `back_seconds`, `back_speech_repeat_count`, `front_display`, `back_display`, `speech_enabled`, `back_speech_rate`, `sort_mode`, and `sort_direction`. The display columns accept `front`, `back`, `transliteration`, `note`, or `image`.
