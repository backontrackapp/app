...
`slug` uses lowercase letters, numbers, and hyphens. `file` is a relative CSV path below the curated directory. `keywords` is pipe-separated. The optional `thumbnail` accepts an HTTPS URL or a relative JPEG, PNG, or WebP path below the curated directory. When present, the storefront shows that image instead of the card-image slideshow. Other optional catalog columns configure the cloned Review set: `mode`, `card_sides`, `indefinite`, `time_limit_seconds`, `max_cards`, `eject_behavior`, `eject_exclude_after`, `front_seconds`, `back_seconds`, `back_speech_repeat_count`, `back_display`, `speech_enabled`, `back_speech_rate`, `sort_mode`, and `sort_direction`.
