...
Emoji fields use the shared `EmojiSelector` component. Its outlined button shows the selected emoji and human-readable icon name, then opens a searchable dialog containing every standard Unicode emoji and skin-tone variant. The search field receives focus when the dialog opens. Results use human-readable labels and a virtual list so only the visible options are mounted. The emoji dataset is loaded the first time a selector without a saved value opens, and saved values preload it so their names can be shown; loaded data is reused by every selector instance.
