...
The POC declares five assistant tools: list the current user's owned Review sets, read matching cards and current-user review statistics from one owned set, propose a new selected-card Review set, propose adding cards to an owned set, and present two to five answer choices. The model cannot invoke undeclared CRUD operations. Read tools run automatically against the local account data. When a question has a discrete answer, its choices appear as buttons below the assistant prompt; the selected choice stays visible in the conversation and is returned to the model. Every create or add proposal shows a preview and requires confirmation; cancellation is returned to the model as a cancelled tool result.
