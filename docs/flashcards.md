...
During TTS playback on the Back card, only the current spoken word receives a quick emphasis pulse. The matching word is emphasized in both the Back value and its transliteration. Front-card speech does not pulse. Browser voices use speech boundary events when available and fall back to a short word timeline; native Android begins that timeline when synthesized audio playback actually starts. Chinese back-language speech colorizes every pinyin syllable and its corresponding Back character by tone, including syllables written together without spaces: first through fourth tones use distinct semantic colors and neutral tone stays muted. Reduced-motion users receive an underline instead of the scale pulse.
