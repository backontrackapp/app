...
Each program-step completion requirement can optionally attach an exercise from the shared RepDB catalog. Its exercise field appears directly above **Completion style**, persists the stable exercise id in that requirement, and is preserved when the step or its task is duplicated. Assigning an exercise clears and hides the requirement's custom name so the exercise becomes its identity. On task cards and the Interval runner's start and completion panels, a requirement uses its custom label as its name when present, otherwise its attached exercise name, then its Interval name. A lightweight bundled presentation index resolves the exercise name and image synchronously from that id, while the full catalog remains lazy-loaded for selection and search. Attached exercise images are therefore selected on first render and loaded eagerly instead of replacing an Interval icon and color afterward. Without an attached or available exercise, Interval requirements retain the saved Interval's color and icon. When a label or exercise supplies the primary name, the Interval name remains visible as secondary context in the runner panels. Program steps can be duplicated from their expanded editor; the copy is inserted on the following program day with independent completion requirements and opens immediately for editing. Check-off, Interval, and Review set requirements can also have an optional label that identifies the requirement on its task card.
